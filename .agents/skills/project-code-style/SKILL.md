...
description: >-
  Use this skill when writing or reviewing Python code in this project. Covers
  import style, naming conventions, performance patterns, ClassVar usage, config
  hierarchy, and dead-code activation checklist.
enabled: true
---
...
- **Only import used names**: Use `from module import name` — never `import module` then call `module.name()`. Only import what is actually used.
...
```python
# ❌ Bad — imports entire module, uses qualified access
import asyncio
...
# ✅ Good — imports only what is used, direct access
...
**Submodule imports** — when a submodule is needed as a namespace (e.g. type hints like `asyncio.subprocess.Process`), import the submodule directly:
```python
from asyncio.subprocess import Process
# then use Process instead of asyncio.subprocess.Process
```

**Multi-name imports** — import all used names from the same module in one line:
```python
from asyncio import CancelledError, Lock, Task, create_task, sleep, wait_for
```
